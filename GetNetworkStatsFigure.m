...
a=2; b=3;
subplot = @(p) subtightplot (2,3, p, [0.06 0.08], [0.06 0.05], [0.1 0.01]);

params=SetParams();
if params.spike_gen.T>1e4
    params.spike_gen.T=1e4;
end
T_view=params.spike_gen.T;
...
semilogy(bins,hist_spikes,'o',bins_shuffled,hist_shuffled,'*')
...
% Export2Folder(['NetworkStats.tif'],target_folder) 