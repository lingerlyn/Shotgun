function [EW, Eb,quality,error_rates,lambda_path]=EstimateA_L1_logistic_cavity_SingleNeuron(CXX,CXY,rates,sparsity,N_stim,pen_diag,pen_dist,warm,true_W,centers)
% Following derivation in GLM_GradUsingSampling 13.1.2015.lyx for a single
% neuron with diagonal Cxx approximation
...
lambda0=2e-5; %initial value  of regularization constant
...
V=-pi*(rates(N).*log(rates(N))+(1-rates(N)).*log(1-rates(N)))/8; %for the gradient
V(isnan(V))=0; %take care of 0*log(0) cases...
L=500*max(V(:))*max(CXX); %lipshitz constant - an educated guess. make sure this is large enough to ensure convergence
...
if pen_diag
    ...
    mask0=ones(1,N); 
    mask0(N)=0; % used to remove L1 penalty from diagonal
end
...
%         mean_U_cavity=bsxfun(@plus,mean_U,y*(CXX*diag((1-rates)./CXX_diag)));
        mean_U_cavity=bsxfun(@plus,mean_U,y.*(1-rates'));
        var_U_cavity=bsxfun(@plus,var_U,-((y.^2).*CXX'));
