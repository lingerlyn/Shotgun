function EW=EstimateA_L1_logistic_sampling(CXX,CXY,rates,sparsity,N_stim,pen_diag,warm)
% Algorithm Implements FISTA algorithm by Beck and Teboulle 2009 for L1 linear regression
% Gradient calculated using sampling - only works now for b=0
...
Tol_FISTA=1/N^2; %tolerance for fista
max_iterations=100;
L_z=1e5; %number of samples
z=bsxfun(@plus,sqrtm(CXX)*randn(N,L_z),rates);  %samples from "spikes"
...
lambda_high=1e1;%max(1e8,1e2*L); %maximum bound for lambda
...
FISTA_cond=1;
iteration=0;
func= @(x) 1./(1+exp(-x)); %logistic function
MXY=CXY+rates*rates'; 
...
while FISTA_cond
    ...
    for nn=1:N
        b=0;  %need to modify this to MLE solution given the weights
        y_nn=y;
        y_nn(:,nn)=0;
        U=bsxfun(@plus,y(:,nn),y_nn*z+b);
        EF(:,nn)=rates(nn)*mean(func(U),2);
    end
