function X=EstimateA_OMP_DistDep(A_,B,spar,tol,lambda,M,rates,PC,lambda2)
...
% PC - inferred probability of a connection
% lambda2 - penalty for connection probability
% Outputs:
...
%             obj=prods-lambda*(zs-M(i,Sc)').^2; %penalize optimal values that are farther away from our means
            obj=prods-lambda*(zs-M(i,Sc)').^2 - lambda2*(1-PC(i,Sc)');
