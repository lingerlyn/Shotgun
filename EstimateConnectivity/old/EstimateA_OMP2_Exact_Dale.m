function EW=EstimateA_OMP2_Exact_Dale(Cxx,Cxy,rates,spar,lambda,M,idents)
%add penalties later.
N=length(Cxx);
...
for a=1:N
    ...
    %       %we have two solutions
        ...
        obj=LSc-lambda*(wSc-repmat(M(a,Sc)',[1,2])).^2 -repmat(abs(idents(Sc)),[1,2]).*(10^50*(sign(wSc)~=repmat(idents(Sc),[1,2])));
