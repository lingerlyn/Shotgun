function EW=EstimateA_Greedy_DistDep_Dale(Cxx,Cxy,rates,spar,lambda,M,PC,lambda2,idents)
%add penalties later.
N=length(Cxx);
...
for a=1:N
    ...
    obj=LSc-repmat(abs(idents(Sc)),[1,4]).*(10^50*(sign(wSc)~=repmat(idents(Sc),[1,4])))-lambda2*repmat(1./(PC(a,Sc))',[1,4]);
    maxval=max(obj(:));
