function EW=EstimateA_L1_logistic_AccurateGrad(CXX,CXY,rates,sparsity,N_stim,pen_diag,warm)
...
L=2*10*max(V(:))*max(eig(CXX)); %lipshitz constant
...
%     A=sqrt(1+(pi/8)*y*CXX*y');    
%     b=A(eye(N)>0.5).*log(rates./(1-rates))-y*rates;
%     if any(isnan(b))
%         error('a neuron had firing rate=0 or 1, so bias is not a number');
%     end
    b=0; %need to modify this to MLE solution given the weights, also requires chaning the gradient...
    
    for nn=1:N
        y_nn=y;
        y_nn(:,nn)=0;
        U=y(:,nn)+y_nn*rates+b;
        A_nn=sqrt(1+(pi/8)*y_nn*CXX*y_nn'); 
        EF(:,nn)=rates(nn)*func(U./A_nn(eye(N)>0.5));
    end
