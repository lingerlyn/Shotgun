function EW=EstimateA_L1_logistic_Accurate_Dale(CXX,CXY,rates,sparsity,N_stim,pen_diag,warm,identities)
...
lambda_low=min(1e-15,1e-2*L);  %minimum bound for lambda
...
while FISTA_cond
    ...
    x=ThresholdOperator(u,mask.*lambda/L,identities,N);
    if any(~isfinite(u(:)))
        error('non finite x!')
    end
...
function y = ThresholdOperator( x , lambda,identities,N )
...
%     if lambda<10^(-5); keyboard; end;
    ...
    %dale's law
    temp2=x.*repmat(identities',[N,1]);
    temp2(~~eye(N))=1; %don't mess with diagonal weights
    y(temp2<0)=0;
