function [W,centers] = construct_weights_realistic_plus1(N,inhib_frac,spar)
%CONSTRUCT_WEIGHTS returns the weight matrix such that
% connectivity is randomly drawn from some distribution
% Also, we add another neuron (index=N) which recieves input from all other
% neurons (the "+1" neuron)

%INPUT
% N = total number of neuron
% spar = sparsity level
...
sgn_array(N)=1; % +1 neuron is excitatory
...
%     p=GetProb(N,spar,ii);
    ...
    if ii<0.9*N %all neurons, except the last ten perscent, are connected to the output neuron
        if conn(end)~=N
            conn(end+1)=N; %#ok . make sure all neurons are conencted to +1 neuron
        end
    else
        if conn(end)==N
            conn(end)=[]; %#ok . make sure all neurons disconnected to +1 neuron
        end
    end
...
%% Do not sort W according to distances - so +1 neuron would have index = N
% chosen_neurons=zeros(N,1)>1; 
% current_neuron=1; %first chosen neuron
% sorted_neurons=[];
% for ii=1:N % do not sort N neuron
